...
normalize_danish_institution <- function(name) {
  name |>
    str_remove_all("\\s*\\([^\\)]+\\)") |>    
    str_remove("^The\\s+") |>                 
    str_squish() |>                           
...
denmark_table <- denmark_table |> 
  mutate(Institution = map_chr(Institution, normalize_danish_institution))
...
normalize_spanish_institution <- function(name) {
  name |>
    ...
    str_squish() |>
    str_replace_all(" - ", ": ") |>
    str_replace_all("^PAIS VASCO$", "UNIVERSIDAD DEL PAIS VASCO") |>
    str_replace_all("^SALAMANCA$", "UNIVERSIDAD DE SALAMANCA") |>
    str_replace_all("^CORUNA$", "UNIVERSIDAD DE LA CORUNA") |>
    str_replace_all("^SEVILLA$", "UNIVERSIDAD DE SEVILLA") |>
    str_replace_all("^OVIEDO$", "UNIVERSIDAD DE OVIEDO") |>
    str_replace_all("^VIGO$", "UNIVERSIDAD DE VIGO") |>
    str_replace_all("^LLEIDA$", "UNIVERSIDAD DE LLEIDA") |>
    str_replace_all("^PABLO DE OLAVIDE$", "UNIVERSIDAD PABLO DE OLAVIDE") |>
    str_replace_all("^CATOLICA DE MURCIA$", "UNIVERSIDAD CATOLICA DE MURCIA") |>
    str_replace_all("^SAN PABLO CEU$", "UNIVERSIDAD SAN PABLO CEU") |>
    str_replace_all("^CEU SAN PABLO$", "UNIVERSIDAD SAN PABLO CEU") |>
    str_replace_all("^MIGUEL HERNANDEZ$", "UNIVERSIDAD MIGUEL HERNANDEZ") |>
    str_replace_all("^POLITECNICA MADRID$", "UNIVERSIDAD POLITECNICA DE MADRID") |>
    str_replace_all("^POLITECNICA CATALUNA$", "UNIVERSIDAD POLITECNICA DE CATALUNA") |>
    str_replace_all("^EUROPEA DE VALENCIA$", "UNIVERSIDAD EUROPEA DE VALENCIA") |>
    str_replace_all("^UVIC.*$", "UNIVERSIDAD VIC") |>
    str_replace_all("^ROVIRA I VIRGIL$", "UNIVERSIDAD ROVIRA I VIRGILI") |>
    str_replace_all("^POMPEU FABRA UNIVERSITY$", "UNIVERSIDAD POMPEU FABRA") |>
    str_replace_all("^CARLOS III DE MADRID$", "UNIVERSIDAD CARLOS III") |>
    str_replace_all("^CASTILLA LA MANCHA$", "UNIVERSIDAD DE CASTILLA LA MANCHA") |>
    str_replace_all("^PUBLICA DE NAVARRA$", "UNIVERSIDAD PUBLICA DE NAVARRA") |>
    str_replace_all("^UNIVERSIDAD DE MIGUEL HERNANDEZ$", "UNIVERSIDAD MIGUEL HERNANDEZ") |>
    str_replace_all("^UNIVERSITAT", "UNIVERSIDAD") |>
    str_replace_all("^UNIVERSITY", "UNIVERSIDAD") |>
    str_squish() |>
    str_to_lower()
}

spain_table <- spain_table |> 
  mutate(University = map_chr(University, normalize_spanish_institution)) 

unique(spain_table$University)
...
normalize_norwegian_institution <- function(name) {
  name |>
    stri_trans_general("Latin-ASCII") |>
    str_remove_all("\\s*\\([^\\)]+\\)") |>
    str_remove_all(",\\s*$") |>
    str_replace_all("\\s*,\\s*", ", ") |>
    str_replace_all("(?i)uiT the arctic university of norway", "University of Tromsø") |>
    str_replace_all("(?i)osloMet – oslo metropolitan university", "Oslo Metropolitan University") |>
    str_replace_all("(?i)BI norwegian business school", "BI Norwegian Business School") |>
    str_replace_all("(?i)NHH norwegian school of economics", "Norwegian School of Economics (NHH)") |>
    str_replace_all("(?i)VID specialized university", "VID Specialized University") |>
    str_replace_all("(?i)MF norwegian school of theology.*", "MF Norwegian School of Theology") |>
    str_replace_all("(?i)Molde university college.*", "Molde University College") |>
    str_replace_all("(?i)NLA university college", "NLA University College") |>
    str_replace_all("(?i)University of south[-–]eastern norway", "University of South-Eastern Norway") |>
    str_replace_all("(?i)Norwegian university of life sciences", "Norwegian University of Life Sciences (NMBU)") |>
    str_replace_all("(?i)Norwegian university of science and technology", "Norwegian University of Science and Technology (NTNU)") |>
    str_replace_all("(?i)Norwegian academy of music", "Norwegian Academy of Music") |>
    str_replace_all("(?i)Norwegian school of sport sciences", "Norwegian School of Sport Sciences") |>
    str_replace_all("(?i)University of inland norway", "Inland Norway University of Applied Sciences") |>
    str_replace_all("(?i)University of agder", "University of Agder") |>
    str_replace_all("(?i)Kristiania university college", "Kristiania University College") |>
    str_replace_all("(?i)Fjellhaug international university college", "Fjellhaug International University College") |>
    str_replace_all("(?i)Oslo school of architecture and design", "Oslo School of Architecture and Design") |>
    str_replace_all("(?i)Nord university.*", "Nord University") |>
    str_replace_all("(?i)University of bergen", "University of Bergen") |>
    str_replace_all("(?i)University of oslo", "University of Oslo") |>
    str_squish()
}

norway_table <- norway_table |> 
  mutate(University = map_chr(University, normalize_norwegian_institution))
...
normalize_polish_institution <- function(name) {
  name |>
    stri_trans_general("Latin-ASCII") |>
    str_remove_all("\\s*\\([^\\)]+\\)") |>
    str_replace_all("Univeristy", "University") |>
    str_replace_all("Od Gdansk", "Of Gdansk") |>
    str_replace_all("Oh John", "Of John") |>
    str_remove_all("(?i)^bachelor studies.*|^engineering and management.*|^food.*|^cinematography.*|^film directing.*|^interdisciplinary.*|^environmental and plant biotechnology.*|^graduate school.*") |>
    str_squish() |>
    str_to_title()
}

poland_table <- poland_table |> 
  mutate(University = map_chr(University, normalize_polish_institution))
...
normalize_slovenian_institution <- function(name) {
  name |>
    stri_trans_general("Latin-ASCII") |>
    str_remove_all("\\s*\\([^\\)]+\\)") |>
    str_remove_all("^The\\s+") |>
    str_remove_all(",.*$") |> 
    str_replace_all("(?i)^University of Ljubljana,?\\s*", "University of Ljubljana, ") |>
    str_replace_all("(?i)^University of Maribor,?\\s*", "University of Maribor, ") |>
    str_replace_all("(?i)^University of Nova Gorica,?\\s*", "University of Nova Gorica, ") |>
    str_replace_all("(?i)^University of Primorska,?\\s*", "University of Primorska, ") |>
    str_replace_all("(?i)^New University,?\\s*", "New University, ") |>
    str_replace_all("(?i)^ALMA MATER EUROPAEA,?\\s*", "Alma Mater Europaea, ") |>
    str_replace_all("(?i)^GEA College,?\\s*", "GEA College, ") |>
    str_replace_all("(?i)^MLC Management and law college,?\\s*", "MLC Management and Law College, ") |>
...
slovenia_table <- slovenia_table |> 
  mutate(Institution = map_chr(Institution, normalize_slovenian_institution)) 
...
normalize_turkish_institution <- function(name) {
  name |>
    stri_trans_general("Latin-ASCII") |>
    str_squish() |>
    str_to_lower() |>
    str_to_title() |>
    str_replace_all("\\bUniversity\\b", "University")
}

turkey_table <- turkey_table |> 
  mutate(University = map_chr(University, normalize_turkish_institution))
...
normalize_belgian_institution <- function(name) {
  name |>
    stri_trans_general("Latin-ASCII") |>
    str_replace_all("[\r\n\t]+", " ") |>
    str_replace_all("\\s{2,}", " ") |>
...
belgium_table <- belgium_table |> 
  mutate(Institution = map_chr(Institution, normalize_belgian_institution))
...
normalize_estonian_institution <- function(name) {
  name |>
    stri_trans_general("Latin-ASCII") |>
    str_remove("/.*$") |>
    str_remove_all("\\s*\\([^\\)]+\\)") |>
...
estonia_table <- estonia_table |> 
  mutate(University = map(University, normalize_estonian_institution))
...
normalize_lithuanian_institution <- function(name) {
  name |>
    stri_trans_general("Latin-ASCII") |>
    str_remove("/.*$") |>
    str_remove_all("\\s*\\([^\\)]+\\)") |>
...
lithuania_table <- lithuania_table |> 
  mutate(University = map_chr(University, normalize_lithuanian_institution)) 
...
normalize_austrian_institution <- function(name) {
  name |>
    stri_trans_general("Latin-ASCII") |>
    str_remove(",.*$") |>
    str_remove_all("\\s*\\([^\\)]+\\)") |>
...
austria_table <- austria_table |> 
  mutate(University_Name = map_chr(University_Name, normalize_austrian_institution))
...
normalize_swedish_institution <- function(name) {
  name |>
    stri_trans_general("Latin-ASCII") |>
    str_remove_all("\\s*\\([^\\)]+\\)") |>
    str_squish() |>
...
sweden_table <- sweden_table |> 
  mutate(University = map_chr(University, normalize_swedish_institution)) 
...
###Germany
```{r}
offsets <- seq(0, 2400, by = 10)
urls <- paste0(
  "https://www2.daad.de/deutschland/studienangebote/international-programmes/en/result/?limit=10&offset=",
  offsets
)

scrape_daad_page <- function(url) {
  message("Scraping: ", url)
  ...
  blocks <- html_elements(html, "div[data-id]")
  if (length(blocks) == 0) return(NULL)

  map_dfr(blocks, function(block) {
    get_text <- function(css) {
      node <- html_element(block, css)
      if (length(node) == 0 || is.na(node)) return(NA)
      html_text(node, trim = TRUE)
    }

    get_field_by_label <- function(label) {
      item <- html_elements(block, "ul.c-ad-carousel__data-list li") %>%
        keep(~ html_element(.x, "h3") %>% html_text(trim = TRUE) == label)
      if (length(item) == 0) return(NA)
      html_element(item[[1]], ".c-ad-carousel__data-item") %>% html_text(trim = TRUE)
    }

    tibble(
      Level         = get_text(".c-ad-carousel__course-type"),
      Institution   = get_text(".js-course-academy"),
      Program_Title = get_text(".js-course-title"),
      Field         = get_field_by_label("Subject"),
      Cost          = get_field_by_label("Costs"),
      Language      = get_field_by_label("Language"),
      Link          = paste0("https://www2.daad.de", html_element(block, "a.js-course-detail-link") %>% html_attr("href"))
    )
  })
}

results <- map(urls, safely(scrape_daad_page))

germany_table <- results %>%
  map("result") %>%
  compact() %>%
  bind_rows()

germany_table <- germany_table %>%
  mutate(
    Level       = gsub("•", "", Level) %>% trimws(),
    Institution = gsub("•", "", Institution) %>% trimws()
  )

normalize_german_institution <- function(name) {
  name |>
    stri_trans_general("Latin-ASCII") |>
    str_remove_all("\\s*\\([^\\)]+\\)") |>        
    str_remove_all("\\s*/.*$") |>                 
    str_remove_all("\\s*\\|.*$") |>               
    str_remove_all("\\s*,.*(University|Hochschule|Institut|College).*") |> 
    str_squish() |>                               
    str_to_lower() |>                             
    str_to_title()                                
}
...
germany_table <- germany_table |> 
  mutate(Institution = map_chr(Institution, normalize_german_institution)) 

View(germany_table)

#write.csv(germany_table, "programs_germany.csv", row.names = FALSE)

length(results)                       # Quante chiamate sono state fatte
sum(map_lgl(results, ~ !is.null(.x$result)))  # Quante hanno avuto successo
...
url <- "https://www.uni.lu/en/education/study-programme-overview/"
page <- read_html(url)
cards <- html_elements(page, ".card__container")

luxembourg_table <- map_dfr(cards, function(card) {
  title <- card %>% html_element(".card__title") %>% html_text(trim = TRUE)
  type  <- card %>% html_element(".card__category") %>% html_text(trim = TRUE)
  topic <- card %>% html_element(".card__topic") %>% html_text(trim = TRUE)
  link  <- card %>% html_element("a.card__link") %>% html_attr("href")
  link  <- if (!is.na(link)) paste0("https://www.uni.lu", link) else NA

  lang_list <- html_elements(card, "ul.card__lang_list li")
...
luxembourg_table <- luxembourg_table %>%
  mutate(
    Languages = case_when(
      str_detect(Field, "Science|Technology|Mathematics|Engineering|Computer") ~ "English, French",
      str_detect(Field, "Law|Economics|Finance")                                ~ "French, English",
      str_detect(Field, "Humanities|Social|Education|Philosophy|Psychology")    ~ "French, German",
      str_detect(Field, "Medicine|Medical|Health|Biomedicine")                  ~ "French, German",
      str_detect(Field, "Continuing Education|Management")                      ~ "English, French",
      TRUE                                                                      ~ NA_character_
    )
  ) %>%
  mutate(Institution = "Université du Luxembourg")

View(luxembourg_table)

#write.csv(luxembourg_table, "programs_luxembourg.csv", row.names = FALSE)

```

###Serbia
```{r}
scrape_programmes_page <- function(page_num) {
  message("Scraping page ", page_num)
  
  url <- paste0("https://www.bazaobrazovanja.rs/en/programmes?page=", page_num)
  page <- tryCatch(read_html(url), error = function(e) return(NULL))
  if (is.null(page)) return(NULL)
  
  rows <- html_elements(page, "tr.programme")
  
  map_dfr(rows, function(row) {
    program_node <- html_element(row, ".resource-title a")
    program_name <- html_text(program_node, trim = TRUE)
    link <- html_attr(program_node, "href")
    full_link <- if (!is.na(link)) paste0("https://www.bazaobrazovanja.rs", link) else NA

    institution <- html_element(row, ".programme-institution-title a") %>% html_text(trim = TRUE)
    language <- html_element(row, ".programme-language") %>% html_text(trim = TRUE)
    level <- html_element(row, "td.type") %>% html_text(trim = TRUE)
...
results <- map(1:178, safely(scrape_programmes_page))

serbia_table <- results %>%
  map("result") %>%
  compact() %>%
  bind_rows()

normalize_serbian_institution <- function(name) {
  name |>
    stri_trans_general("Latin-ASCII") |>
    str_remove_all('\\"[^\\"]*\\"') |>
    str_remove_all("\\s*\\([^\\)]+\\)") |>
...
serbia_table <- serbia_table |> 
  mutate(Institution = map_chr(Institution, normalize_serbian_institution)) 

View(serbia_table)

#write.csv(serbia_table, "programs_serbia.csv", row.names = FALSE)
```
...
programs_EU <- programs_EU |> 
  mutate(Level = case_when(
    str_detect(tolower(Level), "\\bdiploma\\b") ~ "Diploma",  # parola intera
...
known_langs <- c("english", "french", "german", "spanish", "italian", "portuguese",
                 ...
                 "polish", "ukrainian", "armenian", "bulgarian", "serbian", "other", "mixed")
...
#let's see now the number of NAs per variable 
...
countries <- c("Austria", "Estonia", "Spain", "Lithuania", "Sweden", 
               ...
               "Poland", "Denmark", "Croatia", "Germany", "Luxembourg", "Serbia")
...
wanted <- c("PLN", "TRY", "DKK", "NOK", "EUR", "SEK", "RSD")
...
cost_data_joined <- cost_of_living_data |> 
  left_join(exchange_table, by = "Currency") |> 
  mutate(EUR_base = Value * to_EUR)

all_conversions <- exchange_table |> 
  select(Target_Currency = Currency, Rate_to_EUR = to_EUR) |> 
  bind_rows(tibble(Target_Currency = "USD", Rate_to_EUR = 1 / exchange_table$usd_to_currency[exchange_table$Currency == "EUR"]))

converted_data <- cost_data_joined |> 
  crossing(all_conversions) |> 
  mutate(
    Converted_Value = round(EUR_base / Rate_to_EUR, 2)
  ) |> 
  select(Label, Target_Currency, Converted_Value, Country)

cost_of_living <- converted_data |> 
  pivot_wider(
    names_from = Target_Currency,
    values_from = Converted_Value
  ) 

currency_columns <- setdiff(colnames(cost_of_living), c("Label", "Country"))

cost_of_living <- cost_of_living |> 
  select(Label, all_of(sort(currency_columns)), Country)
